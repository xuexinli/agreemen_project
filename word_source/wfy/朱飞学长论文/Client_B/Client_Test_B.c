...
void Save_WP(MYSQL mysql, char w[1024], const EC_POINT *P, const EC_GROUP *group)
{
	...
	strcpy(query_str, "CREATE TABLE Client_B_key(name VARCHAR(20), val VARCHAR(1024));");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('w_A','");	
	...
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('P_A','");
...
void Save_Local_Key_Tab(MYSQL mysql, char KDF_K_BA[50])
{
	char query_str[1024] = {0};
	MYSQL_RES  *res = NULL;
	int rc;
	
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('sk_BA','");	
	strcat(query_str, KDF_K_BA); strcat(query_str, "');");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
...
void udp_PrePhase(int sockfd, struct sockaddr_in addr, const BIGNUM **p_B, const EC_POINT **P_B, const EC_POINT **public_key_C, char w_B[1024], const EC_GROUP *group1, BN_CTX *ctx)
{
	char command[2] = {0};
	/*step1:向CA发送w_B*/
	sendto(sockfd, w_B, strlen(w_B), 0, (struct sockaddr*)&addr, sizeof(addr));
	printf("B已将身份字符串发给CA\n");
	
	/*step2:接收CA的p_B与P_B*/
	char p_str_B[1024] = {0}; char P_str_B[1024] = {0}; char public_key_C_str[1024] = {0};
	struct sockaddr_in addr_t;
    int addrlen_t = sizeof(addr_t);
	int addrlen = sizeof(addr);
	recvfrom(sockfd, p_str_B, 1024, 0, (struct sockaddr *)&addr_t, &addrlen_t);	
	recvfrom(sockfd, P_str_B, 1024, 0, (struct sockaddr *)&addr_t, &addrlen_t);
	recvfrom(sockfd, public_key_C_str, 1024, 0, (struct sockaddr *)&addr_t, &addrlen_t);
	BIGNUM *temp = BN_new(); BN_hex2bn(&temp, p_str_B);*p_B = temp;	
	EC_POINT *temp_d = EC_POINT_new(group1); EC_POINT_hex2point(group1, P_str_B, temp_d, ctx); *P_B = temp_d;	
	EC_POINT *temp_d_2 = EC_POINT_new(group1);EC_POINT_hex2point(group1, public_key_C_str, temp_d_2, ctx); *public_key_C = temp_d_2;
	printf("B已经收到CA发来的第二部分公私钥\n");
    
	/*step3:设备B验证p_B * G = P_B + h_B * C */
	BIGNUM *h_B	= BN_new(); 
	h_generator(w_B, P_str_B, &h_B);
	if(ProveEqual(*p_B, *P_B, h_B, *public_key_C, group1, ctx))
		printf("B验证第二部分公私钥的正确性：验证结果成功，预部署阶段完成\n");
		
}
void udp_Formal(char sk_BA[50], char Auth_Message_A[128], char Auth_Message_B[128], MYSQL mysql, BIGNUM **LK_B,const BIGNUM *x_B,const EC_POINT *X_B,char w_B[1024], const BIGNUM *p_B, const EC_POINT *P_B, int sockfd, struct sockaddr_in addr_A,const EC_POINT *public_key_C,const EC_GROUP *group1, BN_CTX *ctx)
{
    struct sockaddr_in addr_t;
    int addrlen_t = sizeof(addr_t);
	/*step1:设备B向A发送n_B,w_B,P_B,X_B*/ 
	BIGNUM *n_B = BN_new();
	BN_rand(n_B, 160, -1, 1);
	char *n_B_str = BN_bn2hex(n_B), *P_str_B = EC_POINT_point2hex(group1, P_B, 2,NULL), *X_str_B = EC_POINT_point2hex(group1, X_B, 2,NULL);
	sendto(sockfd, n_B_str, strlen(n_B_str), 0, (struct sockaddr*)&addr_A, sizeof(addr_A));
	sendto(sockfd, w_B, strlen(w_B), 0, (struct sockaddr*)&addr_A, sizeof(addr_A));
	sendto(sockfd, P_str_B, strlen(P_str_B), 0, (struct sockaddr*)&addr_A, sizeof(addr_A));	
	sendto(sockfd, X_str_B, strlen(X_str_B), 0, (struct sockaddr*)&addr_A, sizeof(addr_A));	
	printf("B已经向A发送他自己的身份字符串与随机数\n");
	
	/*step2:设备B接收A发送过来的n_A,w_A,P_A并保存*/
	char n_A_str[256] = {0}, P_str_A[512] = {0}, w_A[512] = {0}, X_str_A[512] = {0};
	recvfrom(sockfd, n_A_str, 256, 0, (struct sockaddr *)&addr_t, &addrlen_t);	
	recvfrom(sockfd, w_A, 512, 0, (struct sockaddr *)&addr_t, &addrlen_t);
    //char t_Ag[10] = {0};
    //for(int i = 0; i < 10; i++) t_Ag[i] = w_A[i + 4];
    //time_t t, t_A;
    //time(&t);
    //t_A = atol(t_Ag);
    //if(t - t_A > 2)
    ...
    //    exit(0);
    //}
	recvfrom(sockfd, P_str_A, 512, 0, (struct sockaddr *)&addr_t, &addrlen_t);	
	recvfrom(sockfd, X_str_A, 512, 0, (struct sockaddr *)&addr_t, &addrlen_t);
	BIGNUM *n_A = BN_new(); BN_hex2bn(&n_A, n_A_str);	
	EC_POINT *P_A = EC_POINT_new(group1); EC_POINT_hex2point(group1, P_str_A, P_A, ctx); 
	EC_POINT *X_A = EC_POINT_new(group1); EC_POINT_hex2point(group1, X_str_A, X_A, ctx); 	
	Save_WP(mysql,w_A, P_A, group1);
	printf("B已经收到了A发来的参数（随机数，身份字符串等），并保存\n");
    //char *chan = " chan "; //sprintf(chan, "%ld\0",t-t_A);
    //Print_to_screen(entry, chan);  

	/*step3:设备B检查时间一致性，并计算 preliminary key value K_BA;计算preliminary session key, sk = sk_BA = fai(K_BA);
			准备自己的authentication message = ψ[sk_BA, (w_B,P_B,w_A,P_A,n_B,n_A)]*/ 
	char K_BA[1024] = {0};
	Pre_keyVal(K_BA, p_B, P_A, w_A, public_key_C, x_B, X_A, group1, ctx);	
	KDF(K_BA, sk_BA);
	Save_Local_Key_Tab(mysql, sk_BA);	
	Auth_Message_generator(Auth_Message_B, sk_BA, w_B, P_B, w_A, P_A, n_B, n_A, group1);
	printf("B计算键值与初步会话密钥，并保存初步会话密钥\nB的初步会话密钥 = %s\n",sk_BA);

	/*step4:设备B将自己的authentication message发给A*/ 
	sendto(sockfd, Auth_Message_B, strlen(Auth_Message_B), 0, (struct sockaddr*)&addr_A, sizeof(addr_A));
	printf("B将自己的认证字符串发送给A，认证字符串 = %s\n",Auth_Message_B);

	/*step5:设备B收到设备A的Auth_Message_A,设备B再自己算一下 Auth_Message_A_1，观察是否相等。*/ 
	recvfrom(sockfd, Auth_Message_A, 128, 0, (struct sockaddr *)&addr_t, &addrlen_t);
	printf("B已经收到A的认证字符串 = %s\n",Auth_Message_A);

	char Auth_Message_A_1[128] = {0};
	Auth_Message_generator(Auth_Message_A_1, sk_BA, w_A, P_A, w_B, P_B, n_A, n_B, group1);
	if(strcmp(Auth_Message_A_1, Auth_Message_A) == 0) printf("B认为A可信\n");
		else printf("B认为A不可信\n");

	/*step6:B生成会话密钥LK_B*/ 
	LK_generator(*LK_B, K_BA, n_A, n_B);	
	...
	int rc;
	memset(query_str, 0, 1024);	
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('K_BA','");
	strcat(query_str, K_BA); strcat(query_str, "');");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	char *str1212;str1212 = BN_bn2hex(*LK_B);
	printf("B生成会话密钥 = %s, B将会话密钥保存\n",str1212);

} 
void Save_Repeat(MYSQL mysql, char w_B[1024], const BIGNUM *x_B, const EC_POINT *X_B, const BIGNUM *p_B, const EC_POINT *P_B, const EC_POINT *public_key_C,BIGNUM *LK_B, const EC_GROUP *group, BN_CTX *ctx)
{
	char query_str[1024] = {0};
	MYSQL_RES  *res = NULL;
	int rc;
	...
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('w_B','");	
	...
	temp = BN_bn2hex(x_B);
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('x_B','");		
	strcat(query_str, temp); strcat(query_str, "');");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);	

	temp = EC_POINT_point2hex(group, X_B, 2,NULL);
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('X_B','");
	strcat(query_str, temp); strcat(query_str, "');");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	...
	temp = BN_bn2hex(p_B);
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('p_B','");		
	strcat(query_str, temp); strcat(query_str, "');");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);	
	...
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('public_key_C','");	
	strcat(query_str, temp); strcat(query_str, "');");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	...
	temp = BN_bn2hex(LK_B);
	memset(query_str, 0, 1024);
	strcpy(query_str, "INSERT INTO Client_B_key VALUES ('LK_B','");		
	strcat(query_str, temp); strcat(query_str, "');");
	rc = mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
}
void Get_Repeat(MYSQL mysql, char w_A[1024], char w_B[1024], const BIGNUM **x_B, const EC_POINT **X_B, const BIGNUM **p_B, const EC_POINT **P_B, const EC_POINT **P_A, char K_BA[1024], char sk_BA[50], const EC_POINT **public_key_C, const EC_GROUP *group, BN_CTX *ctx)
{
	char *query_str = NULL;
	MYSQL_RES  *res = NULL;
	MYSQL_ROW  row;	
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"w_A\";";
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"w_B\";";
    mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	row = mysql_fetch_row(res);
	sprintf(temp, "%s", row[0]);
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"x_B\";";
    mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	row = mysql_fetch_row(res);
	sprintf(temp, "%s", row[0]);		
	...
	*X_B = temp_EC;
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"p_B\";";
    mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	row = mysql_fetch_row(res);
	sprintf(temp, "%s", row[0]);		
	...
	*P_B = temp_EC;
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"P_A\";";
    mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	row = mysql_fetch_row(res);
	sprintf(temp, "%s", row[0]);	
	...
	*P_A = temp_EC;
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"K_BA\";";
    mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	row = mysql_fetch_row(res);
	sprintf(temp, "%s", row[0]);		
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"sk_BA\";";
    mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	row = mysql_fetch_row(res);
	sprintf(temp, "%s", row[0]);		
	...
	query_str = "SELECT val FROM Client_B_key WHERE name = \"public_key_C\";";
    mysql_real_query(&mysql, query_str, strlen(query_str));
	res = mysql_store_result(&mysql);
	row = mysql_fetch_row(res);
	sprintf(temp, "%s", row[0]);	
...
_Bool udp_Formal_Repeat(int sockfd, BIGNUM **LK_B,char K_BA[1024], struct sockaddr_in addr_A, char w_A[1024], char w_B[1024], const EC_POINT *P_A, const EC_POINT *P_B, char sk_BA[50],const EC_GROUP *group, BN_CTX *ctx)
{
	struct sockaddr_in addr_t;
    int addrlen_t = sizeof(addr_t);
	...
	BIGNUM *n_B = BN_new();
	BN_rand(n_B, 160, -1, 1);
	char *n_B_str = BN_bn2hex(n_B);
	sendto(sockfd, n_B_str, strlen(n_B_str), 0, (struct sockaddr*)&addr_A, sizeof(addr_A));
	printf("B已经向A发送了新的随机数\n");
	
	/*step2:设备B接收A发送过来的n_A保存*/
	char n_A_str[256] = {0};
	recvfrom(sockfd, n_A_str, 256, 0, (struct sockaddr *)&addr_t, &addrlen_t);	
    printf("B已经收到了A发来的新的随机数\n");
	BIGNUM *n_A = BN_new(); BN_hex2bn(&n_A, n_A_str);	

	/*step3:设备B准备自己的authentication message = ψ[sk_BA, (w_B,P_B,w_A,P_A,n_B,n_A)]*/
	char Auth_Message_B[128] = {0};
	Auth_Message_generator(Auth_Message_B, sk_BA, w_B, P_B, w_A, P_A, n_B, n_A, group);
	...
	/*step5:设备B收到设备A的Auth_Message_A,设备B再自己算一下 Auth_Message_A_1，观察是否相等。*/ 
    printf("B已经收到了A发来的认证信息，并验证它\n");
	char Auth_Message_A[128] = {0};
	recvfrom(sockfd, Auth_Message_A, 128, 0, (struct sockaddr *)&addr_t, &addrlen_t);
	char Auth_Message_A_1[128] = {0};
	Auth_Message_generator(Auth_Message_A_1, sk_BA, w_A, P_A, w_B, P_B, n_A, n_B, group);
	if(strcmp(Auth_Message_A_1, Auth_Message_A) == 0) printf("B认为A可信\n");
		else { printf("B认为A不可信\n"); return 0;}
...
void Third2_page(GtkWidget *widget, gpointer entry)
{
  ...
  {
      /*获得data1中的数据*/
      const BIGNUM *LK_B = (BIGNUM *)(((struct data1 *)entry)->LK_B);
      /*设置show3_box,表示密钥更新或密钥协商阶段过程*/
      GtkWidget *show3_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);gtk_widget_set_size_request(show3_box, 700, 220);//建立垂直布局show3_box 
      ...
      gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B已经向A发送了新的随机数\n", -1, "grey",  NULL);
      num = gtk_text_buffer_get_char_count(buffer3); 
      gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
      gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B已收到了A发来的新的随机数\n", -1, "grey",  NULL);
      num = gtk_text_buffer_get_char_count(buffer3); 
      gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
      gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B将他的认证信息发给A\n", -1, "grey",  NULL);
      num = gtk_text_buffer_get_char_count(buffer3); 
      gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
      gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B已经收到了A发来的认证信息，并验证它\n", -1, "grey",  NULL);
      num = gtk_text_buffer_get_char_count(buffer3); 
      gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
      gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B认为A可信\n", -1, "grey",  NULL);
	  Print_to_screen(buffer3, "新的会话密钥 = ");
         num = gtk_text_buffer_get_char_count(buffer3); 
         gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
         gtk_text_buffer_create_tag(buffer3, "bold", "weight", PANGO_WEIGHT_BOLD, NULL);
         gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, BN_bn2hex(LK_B), -1, "bold",  NULL);
  ...
  /*获得data1中的数据*/
  char sk_BA[50];strcpy(sk_BA,((struct data1 *)entry)->sk_BA);
  ...
  /*代码：与Client_B的密钥协商阶段*/
  /*设置show3_box,表示密钥更新或密钥协商阶段过程*/
  GtkWidget *show3_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);gtk_widget_set_size_request(show3_box, 700, 290);//建立垂直布局show3_box 
  GtkWidget *show3_box_1 = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
  ...
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B已经向A发送他自己的身份字符串与随机数\n", -1, "grey",  NULL);
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B已经收到了A发来的参数（随机数，身份字符串等），并保存\n", -1, "grey",  NULL);
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, "B计算键值与初步会话密钥，并保存初步会话密钥\nB的初步会话密钥 = ", -1, "grey",  NULL);
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, sk_BA, -1, "grey",  NULL);
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, ".\nB将自己的认证字符串发送给A，认证字符串 = ", -1, "grey",  NULL);
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, Auth_Message_B, -1, "grey",  NULL);
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, ".\nB已经收到A的认证字符串 = ", -1, "grey",  NULL); 
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  ...
  //udp_formal
  Print_to_screen(buffer3, "B生成会话密钥 = ");
  num = gtk_text_buffer_get_char_count(buffer3); 
  gtk_text_buffer_get_iter_at_offset(buffer3, &iter, num);
  gtk_text_buffer_create_tag(buffer3, "bold", "weight", PANGO_WEIGHT_BOLD, NULL);
  gtk_text_buffer_insert_with_tags_by_name (buffer3, &iter, BN_bn2hex(LK_B), -1, "bold",  NULL);
  Print_to_screen(buffer3, ", B将会话密钥保存\n");
  gtk_widget_show_all(((struct data1 *)entry)->main_window);
...
void Third_page(GtkWidget *widget, gpointer entry)
{
  ...
  gtk_label_set_markup((GtkLabel *)sublabel,"<span font_desc=\"14.0\">正在与Client_A协商密钥....</span>");//设置sublabel内容 
...
void Second_page(GtkWidget *widget, gpointer entry)
{
  ...
  EC_get_builtin_curves(curves, crv_len); const EC_GROUP *group1 = EC_GROUP_new_by_curve_name(curves[25].nid); Key_generator(key1, group1);
  /*创建一个UDP数据报类型的套接字*/
  int sockfd; struct sockaddr_in addr; struct sockaddr_in addr_B; bzero(&addr_B, sizeof(addr_B));// CA与B的地址
  ...
  /*配置B的地址*/
  addr_B.sin_family = AF_INET; inet_aton("127.0.0.3",&addr_B.sin_addr); addr_B.sin_port = htons(8080);
  if(bind(sockfd, (struct sockaddr*)&addr_B, sizeof(addr_B)) < 0) {fprintf(stderr, "Bind error");exit(1);}
  /*各种参数初始化*/
  const BIGNUM *x_B; const EC_POINT *X_B; const BIGNUM *p_B; const EC_POINT *P_B;
  const EC_POINT *public_key_C; char addr_A_str[1024] = {0};
  struct sockaddr_in addr_t;  int addrlen_t = sizeof(addr_t);
  char w_B[1024] = {0}; BN_CTX *ctx = BN_CTX_new(); BIGNUM *LK_B = BN_new();
  char sk_BA[50] = {0}; char Auth_Message_A[128] = {0}; char Auth_Message_B[128] = {0};
  /*初始化mysql*/
  ...
  if(mysql_real_connect(&mysql, NULL, "root", "123456", NULL, 0, NULL, 0))
	  printf("B成功链接上Mysql\n");
  query_str = "use Client_B"; mysql_real_query(&mysql, query_str, strlen(query_str)); res = mysql_store_result(&mysql);
  query_str = "select * from Client_B_key;"; rc = mysql_real_query(&mysql, query_str, strlen(query_str)); res = mysql_store_result(&mysql);
  ...
  if(rc == 0)//如果是第二次启动
  {
	  char w_A[1024] = {0}, K_BA[1024] = {0}; const EC_POINT *P_A;
      /*获得第一次启动时的各种参数*/
	  Get_Repeat(mysql, w_A, w_B, &x_B, &X_B, &p_B, &P_B, &P_A, K_BA, sk_BA, &public_key_C, group1, ctx);
      /*设备B将自己的地址发给CA*/
      sendto(sockfd, (char*)&addr_B, sizeof(addr_B) + 1, 0, (struct sockaddr*)&addr, sizeof(addr));
      /*设备B接收从CA发来的A的地址*/
      recvfrom(sockfd, addr_A_str, 1024, 0, (struct sockaddr *)&addr_t, &addrlen_t); memcpy(&addr_A,addr_A_str,sizeof(addr_A) + 1);
      /*A与B之间进行第二次通信*/
      if(udp_Formal_Repeat(sockfd, &LK_B,K_BA, addr_A, w_A, w_B, P_A, P_B, sk_BA, group1, ctx) == 0)
      {
      ...
      show_hengxian(show1_box,show1_2_box,frame1_2,show1_2,buffer1_2,provider, "B的预部署阶段\t\t\t\t\t\t\t\t\t\t\t\t\t已完成","/home/wu/桌面/gtk_add/6.2/finish.png");
      ...
      show_hengxian(show1_box,show1_3_box,frame1_3,show1_3,buffer1_3,provider, "B的公钥\t\t\t\t\t\t\t\t\t\t\t\t\t\t","/home/wu/桌面/gtk_add/6.2/lock.png");    
      Print_to_screen(buffer1_3, show_simplekey(EC_POINT_point2hex(group1, X_B, 2,NULL)));
      ...
      show_hengxian(show1_box,show1_4_box,frame1_4,show1_4,buffer1_4,provider, "B的私钥\t\t\t\t\t\t\t\t\t\t\t\t\t\t","/home/wu/桌面/gtk_add/6.2/key.png");    
      Print_to_screen(buffer1_4, show_simplekey(BN_bn2hex(x_B)));           	
      ...
      show_hengxian(show2_box,show2_1_box,frame2_1,show2_1,buffer2_1,provider, "无人机B","/home/wu/桌面/gtk_add/6.2/UAV.png");
      ...
      show_hengxian(show2_box,show2_2_box,frame2_2,show2_2,buffer2_2,provider, "无人机A","/home/wu/桌面/gtk_add/6.2/UAV.png");
  ...
  /*设备B取随机值x_B并将其作为私钥的第一部分，并计算相应的公钥X_B*/
  xX_generator(&x_B, &X_B, group1, ctx);
  /*设备B的ID_B = 0002(十进制)(ID在0-255之间),设备A计算w_B = (ID_B||X_B)*/
  BIGNUM *ID_B = BN_new();
  BN_set_word(ID_B, 0002);
  w_generator(ID_B, X_B, group1, w_B);
  /*调用通信函数与CA进行通信,需要得到p_B,P_B,public_key_C,但是需要输入sockfd,w_B,group1,ctx*/
  udp_PrePhase(sockfd, addr, &p_B, &P_B, &public_key_C, w_B, group1, ctx);
  /*B接收A的地址，以便B与A通信*/
  recvfrom(sockfd, addr_A_str, 1024, 0, (struct sockaddr *)&addr_t, &addrlen_t); memcpy(&addr_A,addr_A_str,sizeof(addr_A) + 1);
  /*A与B相互认证，协商密钥*/ 
  udp_Formal(sk_BA, Auth_Message_A, Auth_Message_B,mysql, &LK_B,x_B,X_B,w_B, p_B, P_B,sockfd,addr_A,public_key_C,group1,ctx);
  /*第一次运行保存所有内容*/
  Save_Repeat(mysql, w_B, x_B, X_B, p_B, P_B, public_key_C, LK_B, group1, ctx);
  ...
  GtkWidget *frame1_2 = gtk_frame_new ("");//初始化边框frame1_2
  GtkWidget *show1_2_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);//建立横向show1_2_box
  show_hengxian(show1_box,show1_2_box,frame1_2,show1_2,buffer1_2,provider, "B的预部署阶段\t\t\t\t\t\t\t\t\t\t\t\t\t刚完成","/home/wu/桌面/gtk_add/6.2/finish.png");
  GtkWidget *show1_3 = gtk_text_view_new();//建立show1_box中的第3个窗口show1_3
  GtkTextBuffer *buffer1_3 = gtk_text_view_get_buffer(GTK_TEXT_VIEW (show1_3));//将buffer1_3连接到show1_3上  
  GtkWidget *frame1_3 = gtk_frame_new("");//初始化边框frame1_3
  GtkWidget *show1_3_box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);//建立横向show1_3_box
  ...
  struct data1 *DATA1 = (struct data1 *)malloc(15000);
  DATA1->page = ((struct data *)entry)->page; DATA1->main_window = ((struct data *)entry)->main_window;
  strcpy(DATA1->sk_BA, sk_BA); strcpy(DATA1->Auth_Message_A, Auth_Message_A); 
  strcpy(DATA1->Auth_Message_B, Auth_Message_B);DATA1->rc = rc; DATA1->LK_B = LK_B;
  /*选择通信对方*/
  GtkWidget *choo_box=gtk_box_new(GTK_ORIENTATION_HORIZONTAL,0);gtk_box_pack_start(GTK_BOX(page2_box),choo_box,0,0, 20);//建立横向choo_box并放入page2_box中
  GdkPixbuf *src5 = gdk_pixbuf_new_from_file("/home/wu/桌面/gtk_add/6.2/com_obj.png", NULL);
  GdkPixbuf *dst5 = gdk_pixbuf_scale_simple(src5,55,55,GDK_INTERP_BILINEAR);
...
static void activate (GtkApplication* app, gpointer user_data)
{
  ...
  gtk_window_set_title (GTK_WINDOW (main_window), "Client_B");//设置main_window名称为Client_A
...
int main (int argc, char **argv)
{
  ...
  GtkApplication *app = gtk_application_new ("org.gtk.example.B", G_APPLICATION_FLAGS_NONE);
